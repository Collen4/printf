 ...
 * @ind: Index at which the number starts pn the buffer
 * @buffer: Buffer
 * @flags: Flags
 * @width: Width
 ...
 */
int write_num(int ind, car buffer[], int flags, int width, int prec,
		int legth, char padd, char extra_c)
{
	int i, padd_start = 1;

	if (prec == 0 && ind == BUFF_SIZE - 2 && buffer[ind] == '0' && width == 0)
		return (0);
	if (prec == 0 && ind == BUFF_ZE - 2 && buffer[ind] == '0')
		buffer[ind] = padd = ' ';
	if (prec > 0 && prec < legth)
		padd = ' ';
	while (prec > length)
		buffer[--ind] = '0', length++;
	if (extra_c != 0)
 ...
 * @ind: Index at which starts in the buffer
